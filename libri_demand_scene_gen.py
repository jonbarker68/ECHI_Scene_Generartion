...
def add_audio(audio, extra_audio, start_id, channel= None):
    ...
    if start_id + extra_len > audio.shape[-1]:
        audio = np.concatenate([audio, np.zeros([audio.shape[0], start_id + extra_len - audio.shape[1]])], axis=1)
    if channel is None:
...
def run(cfg: DictConfig):
    ...
    for i, table in enumerate(convos):
        ...
        file_options = [sorted(glob(audio_dir + f"/{spk}/**/*.flac")) for spk in table_speakers]
        ...
        for ii, c in enumerate(table):
            ...
            if c == 0:
                ...
                new_conv, segs = get_conversation(len(file_options), file_options, block_duration + extra)
                seg_lens += segs
            else:
                # TO DO: generate two conversations to combine
                convA, segsA = get_conversation(2, file_options[:2], block_duration + extra)
                convB, segsB = get_conversation(2, file_options[2:], block_duration + extra)
    ...
    for i, thing in enumerate(convo_signal):
        if thing.shape[1] < max_len:
            ...
            convo_signal[i] = np.concatenate([thing, np.zeros([channels, extra])], axis=1)
    ...
    while mono_noise.shape[-1] <  min_length:
        ...
        mono_noise = np.concat([mono_noise, np.zeros(signal.shape[-1] - transition_samples)])
    ...
    while noise_signal.shape[0] < demand_cfg["n_channels"]:
        noise_signal = np.concatenate([noise_signal, mono_noise[None, :min_length]], axis=0)

    full_audio = np.concatenate([convo_signal, noise_signal], axis=0)

    fig, ax = plt.subplots(tables + 1, 1, figsize=[15, 8])

    xticks = [i for i in range(0, convo_signal.shape[-1], 60 * libri_cfg["sample_rate"])]
    ...
    for i in range(tables):
        ...
        for c, start, end in blocks[i]:
            if c == 1:
                rect = patches.Rectangle((start, 0), end - start, 8, alpha=0.5, facecolor="green", edgecolor="green")
                ax[i].add_patch(rect)
            else:
                colors = ["purple", "blue"]
                for ii in range(c):
                    rect = patches.Rectangle((start, ii * 4), end - start, 4, alpha=0.5, facecolor=colors[ii], edgecolor=colors[ii])
                    ax[i].add_patch(rect)
    ...
    ax[-1].set_yticks(yticks, [f"CH{libri_cfg['n_channels'] + i}" for i in range(demand_cfg["n_channels"])])
